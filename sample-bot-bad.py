...
def main():
    ...
    exchange.send_add_message(order_id=1, symbol="BOND", dir=Dir.SELL, price=999, size=50)
    exchange.send_add_message(order_id=2, symbol="BOND", dir=Dir.BUY, price=1001, size=50)
    ...
    while True:
        ...
        if message["type"] == "close":
            ...
            if message["order_id"] in bond_id_buy_list:
                id_current += 1
                exchange.send_add_message(order_id=id_current, symbol="BOND", dir=Dir.SELL, price=999, size=message["size"])
                bond_id_buy_list.append(id_current)
            if message["order_id"] in bond_id_sell_list:
                id_current += 1
                exchange.send_add_message(order_id=id_current, symbol="BOND", dir=Dir.BUY, price=1001, size=message["size"])
                bond_id_sell_list.append(id_current)
            if message["order_id"] == vale_id_buy_list:
                vale_buy_amt -= message["size"]
            if message["order_id"] == vale_id_sell_list:
            ...
            if message["symbol"] == "VALE":
                ...
                if vale_bid_price == None or valbz_bid_price == None:
                    x = 0
                elif vale_bid_price > valbz_bid_price:
                    id_current += 1
                    exchange.send_cancel_message(order_id=vale_id_sell_list)
                    vale_id_sell_list = id_current
                    exchange.send_add_message(order_id=id_current, symbol="VALE", dir=Dir.BUY, price=vale_bid_price, size=10)

                if vale_ask_price == None or valbz_ask_price == None:
                    x = 0
                elif vale_ask_price < valbz_ask_price and vale_sell_amt != 10:
                    id_current += 1
                    exchange.send_cancel_message(order_id=vale_id_buy_list)
                    vale_id_buy_list = id_current
                    exchange.send_add_message(order_id=id_current, symbol="VALE", dir=Dir.SELL, price=vale_ask_price, size=10)
                ...
                if vale_bid_price == None or valbz_bid_price == None:
                    x = 0
                elif vale_bid_price > valbz_bid_price and vale_buy_amt == 0:
                    id_current += 1
                    exchange.send_cancel_message(order_id=vale_id_sell_list)
                    vale_id_sell_list = id_current
                    exchange.send_add_message(order_id=id_current, symbol="VALE", dir=Dir.BUY, price=vale_bid_price, size=10)
